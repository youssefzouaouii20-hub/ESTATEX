...
const LOCATION_FILES = [
  { name: 'Tunis',      file: '/data/location_tunis.csv' },
  { name: 'La Marsa',   file: '/data/location_lamarsa.csv' },
  { name: 'Ariana',     file: '/data/location_ariana.csv' },
  { name: 'La Manouba', file: '/data/location_lamanouba.csv' },
  { name: 'Sousse',     file: '/data/location_sousse.csv' },
  { name: 'Sfax',       file: '/data/location_sfax.csv' },
  { name: 'Nabeul',     file: '/data/location_nabeul.csv' },
  { name: 'Bizerte',    file: '/data/location_bizerte.csv' },
]
...
function parseCSV(text, cityName) {
  ...
  for(let li=1;li<lines.length;li++){
    ...
    else if(tl.includes('commerce')||tl.includes('local')) type='Commerce'

    // Extract price per month hint
    const isMonthly = rawPrice.toLowerCase().includes('tnd') || rawPrice.toLowerCase().includes('mois')

    rows.push({title,rawPrice,priceNum,location,city:cityName,details,features,desc,image,surface,beds,type,isMonthly})
  }
  return rows
}
...
function RentCard({ listing, fallback, onApply, onDetail, idx }) {
  const imgSrc = proxyImg(listing.image, fallback, listing.type, idx)

  return (
    <div
      onClick={onDetail}
      style={{background:'var(--bg3)',border:'1px solid var(--glass-border)',borderRadius:'var(--radius)',overflow:'hidden',display:'flex',flexDirection:'column',cursor:'pointer',transition:'transform .22s, box-shadow .22s, border-color .22s',isolation:'isolate'}}
    ...
    >
      ...
      <div style={{padding:'16px 18px',flex:1,display:'flex',flexDirection:'column'}}>
        ...
        {(listing.surface>0||listing.beds>0) && (
        ...
        <div style={{marginTop:'auto'}}>
          <div style={{fontSize:'1.1rem',fontWeight:800,color:'var(--accent)',marginBottom:10}}>
            ...
            {listing.priceNum>0 && <span style={{fontSize:'.75rem',color:'var(--text3)',fontWeight:400}}> /mois</span>}
          </div>
          <button
...
export default function Rent() {
  ...
  const [filters, setFilters] = useState({city:'',type:'',search:''})
  ...
  useEffect(()=>{
    if(profile) setAppForm(p=>({...p,full_name:profile.full_name||'',email:profile.email||'',phone:profile.phone||''}))
  },[profile])
  ...
  async function loadAll() {
    setLoading(true)
    const results = await Promise.all(LOCATION_FILES.map(async f => {
  ...
  async function handleApply(e) {
    e.preventDefault()
    setSubmitting(true)
    await supabase.from('rent_applications').insert({
    ...
    setSubmitting(false)
    toast(`✅ Candidature soumise pour "${selected.title}" ! Nous vous contacterons sous 24h.`)
    setSelected(null)
  }
    ...
    <>
      <section style={{background:'var(--bg2)',padding:'60px 24px 40px'}}>
        <div className="container">
          <div className="section-label">Location Immobilière</div>
          <h1 style={{fontSize:'clamp(2rem,5vw,3rem)',fontWeight:800,marginBottom:12}}>Trouver une Location</h1>
          <p style={{color:'var(--text2)',maxWidth:600}}>
            <strong style={{color:'var(--text)'}}>{all.length}</strong> annonces de location à travers toute la Tunisie.
            {loading && ' (Chargement...)'}
          </p>
        </div>
      </section>
          ...
          <div className="filters-bar">
            ...
            <div className="filter-group">
              ...
              <select value={filters.city} onChange={e=>setF('city',e.target.value)}>
                ...
                {LOCATION_FILES.map(f=><option key={f.name} value={f.name}>{f.name}</option>)}
              </select>
            </div>
            <div className="filter-group">
              <label>Type</label>
              <select value={filters.type} onChange={e=>setF('type',e.target.value)}>
                ...
                {['Appartement','Villa','Maison','Studio','Duplex','Bureau','Commerce'].map(t=><option key={t}>{t}</option>)}
              </select>
            </div>
            <button className="btn-ghost" onClick={()=>{setFilters({city:'',type:'',search:''});setPage(1)}}>Réinitialiser</button>
          </div>
        </div>
      </section>

      <section className="section" style={{paddingTop:24}} id="rent-results">
        <div className="container">
          <p style={{color:'var(--text2)',marginBottom:24}}>
            <strong style={{color:'var(--text)'}}>{filtered.length}</strong> propriétés disponibles
          </p>
            ...
            <>
              ...
              {filtered.length===0 && (
                <div className="empty-state">
                  ...
                  <button className="btn-outline" style={{marginTop:16}} onClick={()=>{setFilters({city:'',type:'',search:''});setPage(1)}}>Effacer les filtres</button>
                </div>
              )}
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setDetailOpen(null)}>
          <div className="modal" style={{maxWidth:640,position:'relative'}}>
            ...
            <div style={{borderRadius:'var(--radius2)',overflow:'hidden',marginBottom:20,aspectRatio:'16/9'}}>
              <img src={proxyImg(detailOpen.image, FALLBACKS[0], detailOpen.type, 999)} alt={detailOpen.title}
            ...
            <div style={{fontSize:'1.4rem',fontWeight:800,color:'var(--accent)',marginBottom:16}}>
              {detailOpen.rawPrice||'Prix à consulter'}
              {detailOpen.priceNum>0 && <span style={{fontSize:'.85rem',color:'var(--text3)',fontWeight:400}}> /mois</span>}
            </div>
            {(detailOpen.surface>0||detailOpen.beds>0) && (
            ...
            <button className="btn-primary" style={{width:'100%',justifyContent:'center'}}
              onClick={()=>{setDetailOpen(null);setSelected(detailOpen)}}>
              Postuler maintenant
            </button>
          </div>
        </div>
      ...
      {selected && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setSelected(null)}>
          <div className="modal" style={{position:'relative'}}>
            ...
            <h2>Postuler pour la Location</h2>
            ...
            {selected.rawPrice && <p style={{color:'var(--accent)',fontWeight:700,marginBottom:20}}>{selected.rawPrice}{selected.priceNum>0?' /mois':''}</p>}
            <form onSubmit={handleApply}>
              <div className="form-group"><label>Nom complet *</label><input required value={appForm.full_name} onChange={e=>setAppForm(p=>({...p,full_name:e.target.value}))} /></div>
              <div className="form-row">
                <div className="form-group"><label>Email *</label><input type="email" required value={appForm.email} onChange={e=>setAppForm(p=>({...p,email:e.target.value}))} /></div>
                <div className="form-group"><label>Téléphone</label><input value={appForm.phone} onChange={e=>setAppForm(p=>({...p,phone:e.target.value}))} placeholder="+216 XX XXX XXX" /></div>
              </div>
              <div className="form-group"><label>Message</label><textarea rows={3} value={appForm.message} onChange={e=>setAppForm(p=>({...p,message:e.target.value}))} placeholder="Présentez-vous brièvement..." /></div>
